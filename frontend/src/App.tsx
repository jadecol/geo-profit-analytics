...
type Page = 'dashboard' | 'projects' | 'newProject' | 'analysis' | 'geospatial' | 'sustainability' | 'integrated' | 'reports' | 'advanced';
...
function App() {
  ...
  const handleGeospatialAnalysis = (projectId: number, projectName: string, projectLocation: string) => {
    setNavigationData({ projectId, projectName, projectLocation });
    setCurrentPage('geospatial');
  };

  const handleSustainabilityAnalysis = (projectId: number, projectName: string) => {
    setNavigationData({ projectId, projectName });
    setCurrentPage('sustainability');
  };

  const handleIntegratedAnalysis = (projectId: number, projectName: string, projectLocation: string) => {
    setNavigationData({ projectId, projectName, projectLocation });
        ...
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
          <div style={{
            ...
            <button onClick={() => setCurrentPage('dashboard')} style={{
              padding: '10px 20px', backgroundColor: 'rgba(244, 67, 54, 0.8)',
              color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer',
              marginRight: '20px', backdropFilter: 'blur(10px)'
            }}>
              Dashboard
            </button>
            <button onClick={() => setCurrentPage('newProject')} style={{
              padding: '10px 20px', backgroundColor: 'rgba(76, 175, 80, 0.8)',
              color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer',
              backdropFilter: 'blur(10px)'
            }}>
              Nuevo Proyecto
