import { comparisonService } from "../../services/api"; // AGREGAR este import al inicio
...
const ProjectComparison: React.FC<ProjectComparisonProps> = ({ onBack }) => {
  ...
  const [viewMode, setViewMode] = useState<"matrix" | "radar" | "ranking">(
    "matrix"
  );
  ...
  const runComparison = async () => {
    ...
    try {
      ...
      const response = await comparisonService.compareProjects(projectIds);

      // Transformar datos al formato esperado por los componentes
      const transformedData: ComparisonData = {
        projects: selectedProjects,
        metrics: response.data.metrics.map((metric: any, index: number) => ({
          financial: {
            npv: metric.financial.npv,
            irr: metric.financial.irr,
            roi: metric.financial.roi,
          },
          geospatial: {
            location: metric.geospatial.location,
            accessibility: metric.geospatial.accessibility,
          },
          sustainability: {
            score: metric.sustainability.score,
            carbonFootprint: metric.sustainability.carbonFootprint,
          },
      ...
      alert(
        "Error al comparar proyectos. Revisa la consola para más detalles."
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        >
          <div>
            ...
            <span
              style={{ color: "white", fontSize: "24px", fontWeight: "300" }}
            >
      ...
      <div style={{ padding: "40px", position: "relative", zIndex: 1 }}>
        ...
        {comparisonData && (
          <AnimatePresence mode="wait">
            ...
            {viewMode === "radar" && (
              <RadarComparison key="radar" data={comparisonData} />
            )}
